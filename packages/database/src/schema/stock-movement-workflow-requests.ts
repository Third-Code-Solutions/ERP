...
import {
  stockMovementWorkflowActionEnum,
  stockMovementWorkflowRequestStateEnum,
} from './enums'
import { stockMovements } from './inventory-movements'
import { tenants } from './tenants'
import { users } from './users'

export const stockMovementWorkflowRequests = pgTable(
  ...
  (table) => ({
    ...
    movementTenantFk: foreignKey({
      name: 'stock_movement_workflow_requests_movement_tenant_fk',
    ...
    completedAfterCreatedCheck: check(
      'stock_movement_workflow_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type StockMovementWorkflowRequest =
  typeof stockMovementWorkflowRequests.$inferSelect
export type StockMovementWorkflowRequestInsert =
  typeof stockMovementWorkflowRequests.$inferInsert
