...
import { supplierBillPostRequestStateEnum } from './enums'
import { supplierBills } from './supplier-bills'
import { tenants } from './tenants'
import { users } from './users'

export const supplierBillPostRequests = pgTable(
  'supplier_bill_post_requests',
  {
    ...
    state: supplierBillPostRequestStateEnum('state')
      .notNull()
      .default('processing'),
    result: jsonb('result'),
  ...
  (table) => ({
    ...
    tenantStateIdx: index('idx_supplier_bill_post_requests_tenant_state').on(
      table.tenant_id,
      table.state,
      table.created_at
    ),
    billTenantFk: foreignKey({
      name: 'supplier_bill_post_requests_bill_tenant_fk',
    ...
    completedAfterCreatedCheck: check(
      'supplier_bill_post_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type SupplierBillPostRequest = typeof supplierBillPostRequests.$inferSelect
export type SupplierBillPostRequestInsert =
  typeof supplierBillPostRequests.$inferInsert
