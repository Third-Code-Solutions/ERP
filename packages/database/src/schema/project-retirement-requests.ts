...
import { projectRetirementRequestStateEnum } from './enums'
import { projects } from './projects'
import { tenants } from './tenants'
import { users } from './users'

/** Server-only idempotency ledger for controlled logical project deletion. */
export const projectRetirementRequests = pgTable(
  'project_retirement_requests',
  {
    ...
    state: projectRetirementRequestStateEnum('state')
      .notNull()
      .default('processing'),
    result: jsonb('result'),
  ...
  (table) => ({
    ...
    completedAfterCreatedCheck: check(
      'project_retirement_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`,
    ),
  }),
)

export type ProjectRetirementRequest =
  typeof projectRetirementRequests.$inferSelect
export type ProjectRetirementRequestInsert =
  typeof projectRetirementRequests.$inferInsert
