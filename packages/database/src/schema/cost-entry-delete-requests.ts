...
import { costEntryDeleteRequestStateEnum } from './enums'
import { costEntries } from './cost-entries'
import { projects } from './projects'
import { tenants } from './tenants'
import { users } from './users'

/** Server-only idempotency and restore evidence for cost-entry voids. */
export const costEntryDeleteRequests = pgTable(
  'cost_entry_delete_requests',
  {
    ...
    state: costEntryDeleteRequestStateEnum('state')
      .notNull()
      .default('processing'),
    result: jsonb('result'),
  ...
  (table) => ({
    tenantIdUniqueIdx: uniqueIndex(
      'ux_cost_entry_delete_requests_tenant_id_id'
    ...
    completedAfterCreatedCheck: check(
      'cost_entry_delete_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type CostEntryDeleteRequest = typeof costEntryDeleteRequests.$inferSelect
export type CostEntryDeleteRequestInsert =
  typeof costEntryDeleteRequests.$inferInsert
