...
import {
  purchaseOrderWorkflowActionEnum,
  purchaseOrderWorkflowRequestStateEnum,
} from './enums'
import { purchaseOrders } from './purchase-orders'
import { tenants } from './tenants'
import { users } from './users'

export const purchaseOrderWorkflowRequests = pgTable(
  ...
  (table) => ({
    ...
    purchaseOrderTenantFk: foreignKey({
      name: 'purchase_order_workflow_requests_purchase_order_tenant_fk',
    ...
    completedAfterCreatedCheck: check(
      'purchase_order_workflow_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type PurchaseOrderWorkflowRequest =
  typeof purchaseOrderWorkflowRequests.$inferSelect
export type PurchaseOrderWorkflowRequestInsert =
  typeof purchaseOrderWorkflowRequests.$inferInsert
