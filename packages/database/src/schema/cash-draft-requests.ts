...
import {
  cashTransactionDraftRequestActionEnum,
  cashTransactionDraftRequestStateEnum,
} from './enums'
import { tenants } from './tenants'
import { users } from './users'

/**
 * Durable replay ledger for cash draft mutations.
 * cash_transaction_id is intentionally not an FK: delete-draft must retain
 * replay evidence after the draft row is removed.
 */
export const cashTransactionDraftRequests = pgTable(
  'cash_transaction_draft_requests',
  {
    ...
    action: cashTransactionDraftRequestActionEnum('action').notNull(),
    idempotency_key: varchar('idempotency_key', { length: 256 }).notNull(),
    request_hash: char('request_hash', { length: 64 }).notNull(),
    state: cashTransactionDraftRequestStateEnum('state')
      .notNull()
      .default('processing'),
    result: jsonb('result'),
  ...
  (table) => ({
    ...
    completedAfterCreatedCheck: check(
      'cash_transaction_draft_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type CashTransactionDraftRequest =
  typeof cashTransactionDraftRequests.$inferSelect
export type CashTransactionDraftRequestInsert =
  typeof cashTransactionDraftRequests.$inferInsert
