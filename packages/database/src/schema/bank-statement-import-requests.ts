...
import { bankStatementImportRequestStateEnum } from './enums'
import { tenants } from './tenants'
import { users } from './users'

export const bankStatementImportRequests = pgTable(
  ...
  (table) => ({
    ...
    statementTenantFk: foreignKey({
      name: 'bank_statement_import_requests_statement_tenant_fk',
    ...
    statePayloadCheck: check(
      ...
      sql`(
        (${table.state} = 'processing'
          and ${table.bank_statement_id} is null
          and ${table.result} is null
          and ${table.completed_at} is null)
        or
        (${table.state} = 'succeeded'
          and ${table.bank_statement_id} is not null
          and ${table.result} is not null
          and ${table.completed_at} is not null)
      )`
    ),
    completedAfterCreatedCheck: check(
      'bank_statement_import_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type BankStatementImportRequest =
  typeof bankStatementImportRequests.$inferSelect
export type BankStatementImportRequestInsert =
  typeof bankStatementImportRequests.$inferInsert
