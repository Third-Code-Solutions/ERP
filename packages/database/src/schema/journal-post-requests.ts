...
import { journalPostRequestStateEnum } from './enums'
import { journalEntries } from './accounting'
import { tenants } from './tenants'
import { users } from './users'

export const journalPostRequests = pgTable(
  'journal_post_requests',
  {
    ...
    state: journalPostRequestStateEnum('state')
      .notNull()
      .default('processing'),
    result: jsonb('result'),
  ...
  (table) => ({
    tenantIdUniqueIdx: uniqueIndex('ux_journal_post_requests_tenant_id_id').on(
      table.tenant_id,
      table.id
    ),
    ...
    tenantStateIdx: index('idx_journal_post_requests_tenant_state').on(
      table.tenant_id,
      table.state,
      table.created_at
    ),
    journalEntryTenantFk: foreignKey({
      name: 'journal_post_requests_journal_entry_tenant_fk',
    ...
    completedAfterCreatedCheck: check(
      'journal_post_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type JournalPostRequest = typeof journalPostRequests.$inferSelect
export type JournalPostRequestInsert = typeof journalPostRequests.$inferInsert
