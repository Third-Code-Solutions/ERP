...
import { stockReceiptCreateRequestStateEnum } from './enums'
import { stockReceipts } from './inventory'
import { tenants } from './tenants'
import { users } from './users'

export const stockReceiptCreateRequests = pgTable(
  'stock_receipt_create_requests',
  {
    ...
    state: stockReceiptCreateRequestStateEnum('state')
      .notNull()
      .default('processing'),
  ...
  (table) => ({
    ...
    stockReceiptTenantFk: foreignKey({
      name: 'stock_receipt_create_requests_stock_receipt_tenant_fk',
    ...
    statePayloadCheck: check(
      ...
      sql`(
        (${table.state} = 'processing'
          and ${table.stock_receipt_id} is null
          and ${table.result} is null
          and ${table.completed_at} is null)
        or
        (${table.state} = 'succeeded'
          and ${table.stock_receipt_id} is not null
          and ${table.result} is not null
          and ${table.completed_at} is not null)
      )`
    ),
    completedAfterCreatedCheck: check(
      'stock_receipt_create_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type StockReceiptCreateRequest =
  typeof stockReceiptCreateRequests.$inferSelect
export type StockReceiptCreateRequestInsert =
  typeof stockReceiptCreateRequests.$inferInsert
