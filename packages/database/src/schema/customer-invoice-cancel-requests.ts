...
import { customerInvoiceCancelRequestStateEnum } from './enums'
import { invoices } from './invoices'
import { tenants } from './tenants'
import { users } from './users'

export const customerInvoiceCancelRequests = pgTable(
  'customer_invoice_cancel_requests',
  {
    ...
    state: customerInvoiceCancelRequestStateEnum('state')
      .notNull()
      .default('processing'),
    result: jsonb('result'),
  ...
  (table) => ({
    ...
    completedAfterCreatedCheck: check(
      'customer_invoice_cancel_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type CustomerInvoiceCancelRequest =
  typeof customerInvoiceCancelRequests.$inferSelect
export type CustomerInvoiceCancelRequestInsert =
  typeof customerInvoiceCancelRequests.$inferInsert
