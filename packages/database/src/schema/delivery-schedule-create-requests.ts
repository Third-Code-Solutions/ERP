...
import { deliveryScheduleCreateRequestStateEnum } from './enums'
import { deliverySchedules } from './deliveries'
import { tenants } from './tenants'
import { users } from './users'

export const deliveryScheduleCreateRequests = pgTable(
  'delivery_schedule_create_requests',
  {
    ...
    state: deliveryScheduleCreateRequestStateEnum('state')
      .notNull()
      .default('processing'),
  ...
  (table) => ({
    ...
    deliveryScheduleTenantFk: foreignKey({
      name: 'delivery_schedule_create_requests_schedule_tenant_fk',
    ...
    createdByTenantFk: foreignKey({
      name: 'delivery_schedule_create_requests_created_by_tenant_fk',
      columns: [table.tenant_id, table.created_by],
      foreignColumns: [users.tenant_id, users.id],
    }).onDelete('restrict'),
    keyCheck: check(
      'delivery_schedule_create_requests_key_nonempty',
      sql`${table.idempotency_key} = btrim(${table.idempotency_key})
        and length(${table.idempotency_key}) between 1 and 256`
    ),
    hashCheck: check(
      'delivery_schedule_create_requests_hash_hex',
      sql`${table.request_hash} ~ '^[0-9a-f]{64}$'`
    ),
    resultObjectCheck: check(
      'delivery_schedule_create_requests_result_object',
      sql`${table.result} is null or jsonb_typeof(${table.result}) = 'object'`
    ),
    statePayloadCheck: check(
      'delivery_schedule_create_requests_state_payload',
      sql`(
        (${table.state} = 'processing'
          and ${table.delivery_schedule_id} is null
          and ${table.result} is null
          and ${table.completed_at} is null)
        or
        (${table.state} = 'succeeded'
          and ${table.delivery_schedule_id} is not null
          and ${table.result} is not null
          and ${table.completed_at} is not null)
      )`
    ),
    completedAfterCreatedCheck: check(
      'delivery_schedule_create_requests_completed_after_created',
      sql`${table.completed_at} is null or ${table.completed_at} >= ${table.created_at}`
    ),
  })
)

export type DeliveryScheduleCreateRequest =
  typeof deliveryScheduleCreateRequests.$inferSelect
export type DeliveryScheduleCreateRequestInsert =
  typeof deliveryScheduleCreateRequests.$inferInsert
