...
const AUTH_ORIGIN = 'http://127.0.0.1:4388'
const WEB_ORIGIN = 'http://127.0.0.1:4387'
...
test('proves the authenticated Web receivables page uses Core and preserves tenant-safe rendering', async ({
  page,
}, testInfo) => {
  testInfo.setTimeout(120_000)

  const unauthenticatedPage = await page.request.get(
    `${WEB_ORIGIN}/finance/receivables`,
    { maxRedirects: 0 }
  )
  ...
  const sessionValue = `base64-${Buffer.from(
    JSON.stringify({
      ...
      refresh_token: 'local-finance-receivables-refresh-token',
  ...
  await page.routeWebSocket('ws://127.0.0.1:4388/**', (webSocket) => {
  ...
  await expect(
    page.getByText('INV-LOOPBACK-CURRENT', { exact: true })
  ).toBeVisible()
  await expect(
    page.getByText('INV-LOOPBACK-OVERDUE', { exact: true })
  ).toBeVisible()
  await expect(page.locator('tbody tr')).toHaveCount(2)
  await expect(
    page.locator('.kpi-card-label').filter({ hasText: 'Currently due' })
  ).toBeVisible()
  await expect(
    page.locator('.kpi-card-label').filter({ hasText: 'Past due' })
  ).toBeVisible()
  await expect(
    page.locator('.kpi-card-label').filter({ hasText: 'Retention receivable' })
  ).toBeVisible()

  const bodyText = await page.locator('body').innerText()
  expect(bodyText).toContain('495.00')
  expect(bodyText).toContain('990.00')
  ...
  expect(state.receivablesRequests).toHaveLength(1)
  expect(state.receivablesRequests[0]?.authorization).toBe(
    `Bearer ${session.accessToken}`
  )
  expect(state.receivablesRequests[0]?.requestId).toMatch(
    /^[0-9a-f-]{36}$/i
  )
  const query = new URL(
    state.receivablesRequests[0]!.path,
    'http://core.local'
  ).searchParams
  ...
  for (const viewport of [
    ...
    await page.screenshot({
      path: testInfo.outputPath(`finance-receivables-${viewport.name}.png`),
      fullPage: true,
    })
  }
})
