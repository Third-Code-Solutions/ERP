...
const RUN_SITE_DIARY_MATRIX = process.env.E2E_SITE_DIARY_AUTH === '1'
...
test.describe('site diary role matrix', () => {
  test.skip(
    !RUN_SITE_DIARY_MATRIX || !PROJECT_ID,
    'Set E2E_SITE_DIARY_AUTH=1 and E2E_PROJECT_ID for isolated demo-tenant QA.',
  )

  test('all seeded demo roles can read and only delivery roles see the draft controls', async ({ browser }, testInfo) => {
    ...
    for (const role of selectedRoles) {
      await test.step(role, async () => {
        ...
        try {
          ...
          const response = await page.goto(`${baseUrl}/projects/${PROJECT_ID}/diary`, { waitUntil: 'domcontentloaded' })
          expect(response?.status() ?? 0, role).toBeLessThan(500)
          expect(page.url(), role).not.toMatch(/\/auth\/login/)
          await expect(page.getByRole('heading', { name: 'Daily site diary' }), role).toBeVisible()
          const canManage = ['admin', 'commercial', 'owner', 'safety', 'sd_pm_pe'].includes(role)
          const createButton = page.getByRole('button', { name: 'Create draft' })
          if (canManage) {
            await page.getByText('New daily diary', { exact: true }).click()
