  ...
  return <>
    ...
    {failure || !users.ok || !tenants.ok || !invitations.ok ? <PlatformUnavailable message={failure || 'Platform user data is unavailable.'} /> : <div className="platform-stack">
      ...
      <section className="card"><div className="card-header"><div><h2 className="card-title">User directory</h2><p className="card-subtitle">{users.data.total} users across all tenants</p></div></div>
        {users.data.rows.length === 0 ? <EmptyPlatformState>No users found.</EmptyPlatformState> : <div className="platform-table-wrap" tabIndex={0}><table className="data-table"><caption className="sr-only">Platform user directory and lifecycle controls</caption><thead><tr><th scope="col">User</th><th scope="col">Tenant</th><th scope="col">Status</th><th scope="col">Role</th><th scope="col">Lifecycle</th><th scope="col">Recovery</th></tr></thead><tbody>{users.data.rows.map((user) => <tr key={user.id}>
