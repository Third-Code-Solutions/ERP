...
export async function ProjectFeatureEntry({ feature, searchParams }: {
  ...
  return <div className="grid gap-5">
    ...
    <form method="get" action={`/${feature}`} className="flex flex-wrap items-end gap-3">
      <label className="grid gap-1">Find a project<input className="form-input" type="search" name="q" defaultValue={query.data.q} maxLength={255} placeholder="Project or client name" /></label>
    ...
    {!result.ok || !result.data ? <section className="card p-5" role="alert"><h2>Projects unavailable</h2><p>{result.error || 'The service returned no project data.'}</p><Link href={pageHref(query.data.page)} className="button button-secondary">Retry project list</Link></section> : <>
      <p className="page-subtitle">{result.data.total} projects in your tenant · Page {result.data.page} of {result.data.totalPages}</p>
      {result.data.rows.length === 0 ? <section className="card card-empty" role="status"><h2>No projects found</h2><p>{query.data.q ? 'Try another search or clear the filter.' : 'A project must exist before this workspace can be opened.'}</p><Link href="/projects">Open project directory</Link></section> : <section className="card overflow-x-auto"><table className="data-table"><caption className="sr-only">Choose a project for {config.title}</caption><thead><tr><th scope="col">Project</th><th scope="col">Client</th><th scope="col">Status</th><th scope="col">Workspace</th></tr></thead><tbody>{result.data.rows.map((project) => <tr key={project.id}><td><strong>{project.name}</strong></td><td>{project.client}</td><td>{project.status.replaceAll('_', ' ')}</td><td><Link className="button button-secondary" href={`/projects/${project.id}/${feature}`} aria-label={`Open ${config.title} for ${project.name}`}>Open {config.title}</Link></td></tr>)}</tbody></table></section>}
      <nav className="flex flex-wrap items-center gap-2" aria-label="Project pages">{result.data.page > 1 ? <Link href={pageHref(result.data.page - 1)} className="button button-secondary">Previous</Link> : null}{result.data.page < result.data.totalPages ? <Link href={pageHref(result.data.page + 1)} className="button button-secondary">Next</Link> : null}</nav>
    </>}
  </div>
}
