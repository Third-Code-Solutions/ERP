...
vi.mock('next/navigation', () => ({
  usePathname: () => '/projects/33333333-3333-4333-8333-333333333333/documents',
}))
...
describe('project document action visibility', () => {
  ...
  it.each(ERP_ROLES)('matches server document.manage policy for %s', async (role) => {
    ...
    const page = await ProjectDocumentsPage({ params: Promise.resolve({ id: projectId }) })
    const markup = renderToStaticMarkup(<>
      <ProjectTabs projectId={projectId} access={getProjectDetailAccess(role)} />
      {page}
    </>)
    ...
    expect(markup).toContain('<caption class="sr-only">Project documents for Fixture project</caption>')
    expect(markup).toContain('aria-label="Documents table for Fixture project, scroll for more columns"')
    ...
    expect(markup.match(/>Documents<\/a>/g)).toHaveLength(1)
