 ...
 * live project dashboard without an ABI login. Distinct from
 * `/portal/sign` (short-lived, one-shot signing) and `/portal/warranty`
 * (one-link-per-project, ticket submission only).
 *
 * Pattern mirrors `apps/web/src/lib/abi/integrations/canvas-sign.ts`:
...
import { customerPortalSessions } from '@buildops/database/schema'
