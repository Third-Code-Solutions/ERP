...
async function seedFixture(transaction: DatabaseTransaction) {
  ...
  await transaction.execute(sql`
    ...
    values (${tenantId}::uuid, ${`Supplier bill reverse ${suffix}`}, ${`supplier-bill-reverse-${suffix}`})
  `)
  await transaction.execute(sql`
  ...
  await transaction.execute(sql`
    ...
    values (${projectId}::uuid, ${tenantId}::uuid, 'Supplier Bill Project', 'Owner', ${actorId}::uuid)
  `)
  await transaction.execute(sql`
  ...
  return { tenantId, actorId, viewerId, billId, suffix }
}

suite('Supplier Bill reverse database integration', () => {
  it('posts, reverses once, replays safely, audits, and denies a viewer', async () => {
    try {
      await db.transaction(async (transaction) => {
        ...
        const principal: ErpPrincipal = {
          ...
          role: 'finance',
          email: `finance-${fixture.suffix}@integration.test`,
        }
        await new SupplierBillPostService(
          new ConfigService({
            ERP_FINANCE_SUPPLIER_BILL_POST_WRITES_ENABLED: true,
            ERP_FINANCE_SUPPLIER_BILL_POST_WRITES_TENANT_IDS: [fixture.tenantId],
          }),
          bound,
          audit
        ).post(
          ...
          principal,
          'supplier-bill-reverse-post-1'
        )

        const service = new SupplierBillReverseService(
          new ConfigService({
            ERP_FINANCE_SUPPLIER_BILL_REVERSE_WRITES_ENABLED: true,
            ERP_FINANCE_SUPPLIER_BILL_REVERSE_WRITES_TENANT_IDS: [fixture.tenantId],
          }),
          bound,
          audit
        )
        ...
        const first = await service.reverse(
          ...
          command,
          principal,
          'supplier-bill-reverse-integration-1'
        )
        ...
        const [bill] = await rows(
          transaction,
          sql`select status, reversal_journal_entry_id, reversal_reason
        ...
        const [request] = await rows(
          ...
          sql`select state, result
              ...
              where tenant_id = ${fixture.tenantId}::uuid
                and idempotency_key = 'supplier-bill-reverse-integration-1'`
        )
        ...
        expect(bill).toMatchObject({
          status: 'reversed',
          reversal_journal_entry_id: first.reversalJournalEntryId,
          reversal_reason: command.reason,
        })
        expect(request).toMatchObject({ state: 'succeeded', result: first })
        expect(auditRows.some((row) => row.action === 'status_change')).toBe(true)
        ...
        await expect(
          service.reverse(
            ...
            command,
            {
              ...
              role: 'viewer',
              email: `viewer-${fixture.suffix}@integration.test`,
            },
            'supplier-bill-reverse-viewer-1'
          )
        ).rejects.toThrow()
        throw ROLLBACK
      })
