...
describe('Finance receivables HTTP contract', () => {
  ...
  async function appFor(list = vi.fn()) {
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [{ provide: FinanceReceivablesService, useValue: { list } }],
    }).compile()
  ...
  it('rejects unsupported query fields before the service', async () => {
    ...
    await request(app.getHttpServer())
      .get('/v1/finance/receivables?unexpected=true')
      .expect(400)
  ...
  it('forwards bounded filters and the verified finance principal', async () => {
    ...
    await request(app.getHttpServer())
      .get(
        '/v1/finance/receivables?accountId=55555555-5555-4555-8555-555555555555&dueFrom=2026-08-01&dueTo=2026-08-31&page=2&limit=100'
      )
      .expect(200)
