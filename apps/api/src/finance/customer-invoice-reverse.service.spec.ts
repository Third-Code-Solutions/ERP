...
function service(enabled = false, tenantIds: string[] = []) {
  const config = {
    get: vi.fn((key: string) =>
      key === 'ERP_FINANCE_CUSTOMER_INVOICE_REVERSE_WRITES_ENABLED'
        ? enabled
        : tenantIds
    ),
  ...
  return {
    service: new CustomerInvoiceReverseService(config, database, {} as AuditService),
    transaction,
  }
}

describe('CustomerInvoiceReverseService migration boundary', () => {
