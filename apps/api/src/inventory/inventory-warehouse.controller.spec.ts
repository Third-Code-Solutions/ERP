...
describe('Inventory Warehouse HTTP contract', () => {
  ...
  async function appFor(create = vi.fn()) {
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        {
          provide: InventoryWarehouseCreationService,
          useValue: { create },
        },
      ],
    }).compile()
  ...
  it('forwards validated command and verified principal', async () => {
    ...
    await request(app.getHttpServer())
      .post('/v1/inventory/warehouses')
      .send({ code: ' MAIN ', name: ' Main store ', projectId: null })
      .expect(201)

    expect(create).toHaveBeenCalledWith(
      { code: 'MAIN', name: 'Main store', projectId: null },
