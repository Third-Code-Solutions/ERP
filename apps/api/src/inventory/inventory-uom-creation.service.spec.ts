...
import type { CreateInventoryUomCommand } from '@third-code-erp/shared-types'
...
const COMMAND: CreateInventoryUomCommand = {
  code: 'EA',
  name: 'Each',
  decimalPlaces: 0,
}
...
function service(
  ...
  const config = {
    get: vi.fn((key: string) =>
      key === 'ERP_INVENTORY_UOM_CREATE_WRITES_ENABLED'
        ? enabled
        : tenantIds
    ),
  ...
  return {
    candidate: new InventoryUomCreationService(
      config,
      database,
...
describe('InventoryUomCreationService migration boundary', () => {
  ...
  it('stays disabled when tenant allowlist is empty', async () => {
    await expect(service(true).candidate.create(COMMAND, PRINCIPAL)).rejects.toThrow(
      'Inventory UOM creation is not enabled for this tenant; no UOM was changed.'
    )
  })

  it('locks membership, creates a tenant UOM, and audits the result', async () => {
    const membership = {
