...
const environmentSchema = z.object({
  ...
  ERP_CORTEX_BRIEF_READS_ENABLED: z
    ...
    .default('false')
    .transform((value) => value === 'true'),
  ERP_CORTEX_BRIEF_READS_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
  ...
  ERP_FINANCE_CASH_READS_TENANT_IDS: z
    ...
    .pipe(z.array(z.string().uuid())),
  // Bank reconciliation register reads stay fail-closed until statement,
  // cash-account, RLS, and protected canary parity are approved.
  ERP_FINANCE_RECONCILIATION_READS_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  ERP_FINANCE_RECONCILIATION_READS_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
    ...
    .pipe(z.array(z.string().uuid())),
  // Auto-match writes stay fail-closed until the request ledger, trusted
  // function parity, and a protected tenant canary are approved.
  ERP_FINANCE_RECONCILIATION_AUTO_MATCH_WRITES_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  ERP_FINANCE_RECONCILIATION_AUTO_MATCH_WRITES_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
  ...
  ERP_FINANCE_RECONCILIATION_LINE_MATCH_WRITES_TENANT_IDS: z
    ...
    .pipe(z.array(z.string().uuid())),
  // Reconciliation writes stay fail-closed until the request ledger, trusted
  // function parity, and a protected tenant canary are approved.
  ERP_FINANCE_RECONCILIATION_RECONCILE_WRITES_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  ERP_FINANCE_RECONCILIATION_RECONCILE_WRITES_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
  ...
  ERP_FINANCE_RECONCILIATION_VOID_WRITES_TENANT_IDS: z
    ...
    .pipe(z.array(z.string().uuid())),
  // Bank-statement import writes stay fail-closed until the request ledger,
  // parser parity, trusted transaction, and protected tenant canary are
  // approved.
  ERP_FINANCE_RECONCILIATION_IMPORT_WRITES_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  ERP_FINANCE_RECONCILIATION_IMPORT_WRITES_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
  ...
  ERP_PROJECT_CREATE_WRITES_TENANT_IDS: z
    ...
    .pipe(z.array(z.string().uuid())),
  // Project discussion creation stays fail-closed until the Core transaction,
  // browser-DML revocation, replay, and protected tenant canary are approved.
  ERP_PROJECT_COMMENT_CREATE_WRITES_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  ERP_PROJECT_COMMENT_CREATE_WRITES_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
  ...
  ERP_PROJECT_COMMENT_DELETE_WRITES_TENANT_IDS: z
    ...
    .pipe(z.array(z.string().uuid())),
  // Project retirement is an evidence-preserving logical deletion. It stays
  // closed until its additive migration and one protected-tenant replay pass.
  ERP_PROJECT_DELETE_WRITES_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  ERP_PROJECT_DELETE_WRITES_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
  ...
  ERP_OPPORTUNITY_CONVERT_WRITES_TENANT_IDS: z
    ...
    .pipe(z.array(z.string().uuid())),
  // Opportunity stage changes stay fail-closed until the transition ledger,
  // SLA/audit transaction, and atomic won handoff canary are approved.
  ERP_OPPORTUNITY_STAGE_WRITES_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((value) => value === 'true'),
  ERP_OPPORTUNITY_STAGE_WRITES_TENANT_IDS: z
    .string()
    .default('')
    .transform((value) =>
      value
