...
describe('ERP API environment', () => {
  ...
  it('keeps project retirement disabled and tenant-scoped by default', () => {
    const defaults = validateEnvironment(REQUIRED)
    expect(defaults.ERP_PROJECT_DELETE_WRITES_ENABLED).toBe(false)
    expect(defaults.ERP_PROJECT_DELETE_WRITES_TENANT_IDS).toEqual([])

    expect(
      validateEnvironment({
        ...REQUIRED,
        ERP_PROJECT_DELETE_WRITES_ENABLED: 'true',
    ...
    expect(() =>
      validateEnvironment({
        ...REQUIRED,
        ERP_PROJECT_DELETE_WRITES_TENANT_IDS: 'not-a-tenant',
      })
    ).toThrow('ERP_PROJECT_DELETE_WRITES_TENANT_IDS')
  })
