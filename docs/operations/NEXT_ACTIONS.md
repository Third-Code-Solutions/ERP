...
## Exact next action after M3.299 Core Purchase Order from approved BOM canary

Keep `ERP_PO_BOM_CREATE_WRITES_VIA_API=false` and
`ERP_PO_BOM_CREATE_WRITES_VIA_API_TENANT_IDS` empty; keep
`ERP_PO_BOM_CREATE_WRITES_ENABLED=false` and
`ERP_PO_BOM_CREATE_WRITES_TENANT_IDS` empty outside disposable tests. The
local proof covers approved-BOM scope, exact totals, copied-line provenance,
BOM lock, idempotent replay, audit, tenant isolation, and cleanup. Do not open
a real tenant, apply hosted SQL, or trigger Vercel/Railway builds until the
hosted PO duplicate owner review, managed parity, readiness, rollback,
authenticated smoke, release identity, and spend gates pass.
...
Keep
`ERP_FINANCE_RECONCILIATION_IMPORT_STORAGE_UPLOADS_ENABLED=false` and
`ERP_FINANCE_RECONCILIATION_IMPORT_STORAGE_UPLOADS_TENANT_IDS` empty in Core;
keep the Web `ERP_FINANCE_RECONCILIATION_IMPORT_STORAGE_UPLOADS_VIA_API=false`
and its tenant list empty, with the existing Web form/Storage selector also
closed. Build a disposable authenticated browser canary that records the Core
sign/cleanup requests, proves bearer/request-id/RBAC/tenant-prefix behavior,
and leaves zero Storage fixtures. Then reconcile managed Storage RLS/key
parity, readiness, exact release identity, rollback, authenticated smoke, and
spend evidence. Do not apply hosted SQL, change provider settings, or trigger
Vercel/Railway builds.

## Exact next action after M3.278 protected bank-reconciliation Web/Core browser canary

Keep `ERP_FINANCE_RECONCILIATION_READS_VIA_API=false` and
`ERP_FINANCE_RECONCILIATION_READS_VIA_API_TENANT_IDS` empty; keep
`ERP_FINANCE_RECONCILIATION_READS_ENABLED=false` and
`ERP_FINANCE_RECONCILIATION_READS_TENANT_IDS` empty. The disposable browser
proof now covers authenticated Core selection, bearer/request-id/query
forwarding, exact statement/KPI/register rendering, redirect/RBAC, blocked
provider traffic, console/page errors, responsive layout, and cleanup. Source
parity remains 55/124 with 69 pending migrations in 17 ordered review batches.
Next gates are hosted/source parity, readiness, exact release identity,
authenticated production smoke, rollback, and spend control. Do not apply
hosted SQL or trigger Vercel, Railway, or provider builds.
...
`ERP_FINANCE_CASH_READS_VIA_API_TENANT_IDS` empty; keep
`ERP_FINANCE_CASH_READS_ENABLED=false` and
`ERP_FINANCE_CASH_READS_TENANT_IDS` empty. The disposable Nest HTTP canary
proves auth/RBAC, invalid-query and selector failure, exact receipt/
disbursement totals and status counts, direction/date/account filters,
pagination, row ordering, tenant isolation, and rollback. Source parity
remains 55/124 with 69 pending migrations in 17 ordered review batches. Next
add a separate authenticated browser proof for the real `/finance/cash` page,
...
## Exact next action after M3.275 protected finance-payables Web/Core browser canary
...
`ERP_FINANCE_PAYABLES_READS_TENANT_IDS` empty. The disposable browser proof
now covers authenticated Core selection, bearer/request-id/query forwarding,
exact supplier-bill KPI/aging/table rendering, redirect/RBAC, blocked provider
traffic, console/page errors, responsive layout, and cleanup. Source parity
remains 55/124 with 69 pending migrations in 17 ordered review batches. Next
gates are hosted/source parity, readiness, exact release identity,
authenticated production smoke, rollback, and spend control. Do not apply
hosted SQL or trigger Vercel, Railway, or provider builds.
...
`ERP_FINANCE_RECEIVABLES_READS_VIA_API_TENANT_IDS` empty; keep
`ERP_FINANCE_RECEIVABLES_READS_ENABLED=false` and
`ERP_FINANCE_RECEIVABLES_READS_TENANT_IDS` empty. The disposable browser
proof now covers authenticated Core selection, bearer/request-id/query
forwarding, exact invoice/KPI rendering, redirect/RBAC, blocked provider
traffic, console/page errors, responsive layout, and cleanup. Source parity
remains 55/124 with 69 pending migrations in 17 ordered review batches. Next
gates are hosted/source parity, readiness, exact release identity,
authenticated production smoke, rollback, and spend control. Do not apply
...
`ERP_FINANCE_LEDGER_READS_TENANT_IDS` empty. The local Nest HTTP canary proves
auth/RBAC, tenant isolation, exact centavo totals, filters, pagination,
selector failure, and rollback only against disposable PostgreSQL. Source
parity remains 55/124 with 69 pending migrations in 17 ordered review batches.
Next gates are hosted/source parity, readiness, exact release identity,
...
`ERP_FINANCE_RECONCILIATION_IMPORT_WRITES_TENANT_IDS` empty. The local Core
canary, shared/database contracts, full package tests, API integration,
typecheck, lint, production build, policy, parity, release, boundary,
workflow, actionlint, and spend gates pass. Source parity is 55/123 with 68
pending migrations in 16 ordered review batches; managed Supabase and
provider deployments remain untouched. Next source boundary: storage-backed
upload plus Web/Core response parity, with its own rollback-only canary.
After that, separately require hosted parity, release identity, readiness,
protected browser, rollback, and spend evidence. Do not apply hosted SQL or
trigger Railway/Vercel builds.
...
`ERP_FINANCE_RECONCILIATION_RECONCILE_WRITES_TENANT_IDS` empty. The local Core
canary, strict shared/database contracts, full package tests, API integration,
typecheck, lint, production build, policy, parity, release, boundary,
workflow, actionlint, and spend gates pass. Source parity is 55/121 with 66
pending migrations in 14 ordered review batches; managed Supabase and provider
deployments remain untouched. Next source boundary: void or import authority
with its own ledger and rollback-only canary. After that, separately require
hosted parity, release identity, readiness, protected browser, rollback, and
spend evidence. Do not apply hosted SQL or trigger Railway/Vercel builds.

## Exact next action after M3.262 bank-statement line match/unmatch authority

Keep `ERP_FINANCE_RECONCILIATION_LINE_MATCH_WRITES_ENABLED=false` and
`ERP_FINANCE_RECONCILIATION_LINE_MATCH_WRITES_TENANT_IDS` empty. The local
Core canary, strict shared/database contracts, full package tests, API
integration, typecheck, lint, direct builds, policy, parity, release,
boundary, workflow, actionlint, and spend gates pass. Source parity is 55/120
with 65 pending migrations in 13 ordered review batches; managed Supabase and
provider deployments remain untouched. Next source boundary: reconcile/void
or import authority with its own ledger and rollback-only canary. After that,
separately require hosted parity, release identity, readiness, protected
browser, rollback, and billing evidence. Do not apply hosted SQL or trigger
Railway/Vercel builds.
...
`ERP_FINANCE_JOURNAL_POST_WRITES_TENANT_IDS` empty. Focused canary passes 1/1;
API integration passes 50/50 files and 64 tests with two explicit
Redis-restart skips under the 15-second timeout; static, build, release,
boundary, workflow, actionlint, and spend gates pass. Do not apply hosted SQL
or trigger Railway/Vercel builds while spend protection and the M3.248 hosted
parity/security gate remain active. Next source boundary: journal-reversal
authority, then reconcile hosted parity, release identity, readiness,
protected browser evidence, rollback, and billing gates.

## M3.255 evidence boundary (completed source-only canary)

The protected journal-post canary passes 1/1 and the API integration lane
passes 50/50 files and 64 tests with two explicit Redis-restart skips. It
covers tenant preflight, auth/RBAC, disabled selector, cross-tenant
concealment, idempotency, balanced posting, audit, tenant isolation, and
rollback. No provider or paid action occurred.

## Exact next action after M3.254 supplier-bill reversal canary

Keep `ERP_FINANCE_SUPPLIER_BILL_REVERSE_WRITES_ENABLED=false` and
`ERP_FINANCE_SUPPLIER_BILL_REVERSE_WRITES_TENANT_IDS` empty. Focused canary
passes 1/1; API integration passes 49/49 files and 63 tests with two explicit
Redis-restart skips under the 15-second timeout; static, build, release,
boundary, workflow, actionlint, and spend gates pass. Do not apply hosted SQL
or trigger Railway/Vercel builds while spend protection and the M3.248 hosted
parity/security gate remain active. Next source boundary: journal-post
authority, then reconcile hosted parity, release identity, readiness, protected
browser evidence, rollback, and billing gates.

## M3.254 evidence boundary (completed source-only canary)

The protected supplier-bill reverse canary passes 1/1 and the API integration
lane passes 49/49 files and 63 tests with two explicit Redis-restart skips. It
covers strict input, auth/RBAC, disabled selector, cross-tenant concealment,
legal reversal state, idempotency, balanced journal unwind, audit, tenant
isolation, and rollback. No provider or paid action occurred.
...
Protected Stock Receipt database and HTTP canaries passed 2/2. Evidence covers
real auth/capability guards, strict command/header validation, disabled-tenant
fail-closed behavior, exact PO/material/UOM/warehouse scope, idempotent
replay/key conflict, receipt-line persistence, semantic audit, RLS/browser
privilege boundaries, cross-tenant concealment, and rollback. No
...
authorization, state-machine, transaction, idempotency, audit, and rollback
contracts; implement the narrowest change; then run focused checks, root source
gates, and the disposable PostgreSQL/Redis lane. Keep project-comment,
notification, and Today selectors false, all other Core selectors false/empty,
and do not mutate Supabase or trigger Vercel/Railway builds while the spend
lock is active.

## M3.234 evidence boundary (completed)

The protected local project-comment canary passed 1/1. It proved identity,
viewer denial, tenant/project isolation, mention resolution, idempotent replay
and conflict, request correlation, audited create/delete, terminal
disablement, and rollback. The full disposable environment passed 116
migrations, database 149/149 suites and 370/370 tests, API integration 66/66
suites and 49/49 tests, zero pending/skips, and unchanged schema SHA-256.
Hosted selector, browser production canary, provider state, deployment, and
billing remain unverified and intentionally untouched.
...
authorization, state, transaction, idempotency, audit, and rollback contracts;
write the narrowest change; then run focused checks, root source gates, and the
disposable PostgreSQL/Redis lane. Keep `ERP_TODAY_READS_VIA_API=false`, all
other Core selectors false/empty, and do not mutate Supabase or trigger
Vercel/Railway builds while the spend lock is active.

## M3.232 evidence boundary (completed)

The protected local `/v1/today` canary passed 2/2. It proved identity,
tenant/current-assignee and cross-tenant filtering, project expansion,
strict browser-time rejection, request-id propagation, unsupported-role
403, and rollback. The full disposable lane passed 116 migrations, database
149/149 suites and 370/370 tests, API integration 62/62 suites and 47/47
tests, zero pending/skips, and unchanged schema SHA-256. Hosted selector,
browser production canary, provider state, deployment, and billing remain
unverified and intentionally untouched.
...
1. Source commit `c4d618545495dd0c174c4ddd9d43655cc8cdcd38` is pushed under
   `kurtgav` and its exact remote SHA matches. Do not deploy, reconnect Vercel
   Git, or change provider plans.
2. Keep API and Web void selector pairs false/empty outside the disposable
   tenant. The next reconciliation authority slice is bank-statement import,
   as a separate idempotent command.
3. Hosted promotion remains blocked by migration parity, duplicate Purchase
   Order mapping, audit-recovery approval, readiness, rollback, exact SHA, and
   spend-bounded owner/provider approval. No paid/duplicate build is allowed
   to bypass those gates.
