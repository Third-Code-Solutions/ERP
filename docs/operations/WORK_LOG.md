...
harness. It seeds a primary portal token and a same-tenant token aimed at a
foreign BOM, sends a secret-protected completion callback twice, and records
the internal Core token/request ID without exposing the secret. Nest Core
marks the token used, stores one signed contract document, locks the BOM, and
writes one semantic audit; replay is duplicate-only and the foreign BOM stays
draft. Resend is forced into its local stub, so no paid email/provider call is
made.

The first run found a real Postgres `FOR UPDATE` error caused by locking the
nullable side of a `LEFT JOIN`. Replaced the required project relation with an
inner join and reran the proof successfully.

Selectors remain closed. No hosted Supabase SQL, Storage, Railway/Vercel
deployment, credential, provider setting, or paid action changed.

Validation: DocuSeal browser 1/1; existing Togal/notification/document
browser canaries 1/1 each; API 176/772; Web 113/802; focused route/Core,
...
blocked on owner remediation, and review the next Core boundary only after
readiness, rollback, authenticated smoke, and spend gates pass.
...
`/finance/reconciliation` page. The harness starts disposable PostgreSQL, the
compiled Nest API, loopback Supabase-compatible auth/profile, and a
request-recording Core proxy. An authenticated tenant renders draft,
reconciled, and voided statement rows with exact closing balances and match
progress; the browser asserts bearer/request-id/query forwarding, redirect,
RBAC, blocked font traffic, no unexpected contracts or console/page errors,
desktop/mobile overflow, and cleanup. Workflow authority remains separately
covered by the Core HTTP canary.

Reconciliation browser 1/1 by direct config and package script, Web 113/782,
root tests/typecheck/lint/build, provider-spend, boundary, workflow,
actionlint, gitleaks, release, and managed-parity gates pass. No hosted
Supabase, Storage, Railway, Vercel, credential, provider, or paid action
changed. Final matching fixture count is zero.
...
Supabase-compatible auth/profile, and a request-recording Core proxy. An
authenticated tenant renders two posted supplier bills and one draft bill with
exact PHP KPI/aging/table values; the browser asserts bearer/request-id/query
forwarding, redirect, RBAC, blocked font traffic, no unexpected contracts or
console/page errors, desktop/mobile overflow, and cleanup.

Payables browser 1/1, Web 113/782, root tests/typecheck/lint/build,
...
rollback-only Nest HTTP canary boots the real controller/service with JWT
identity and capability guards, two tenants, finance/viewer roles, posted
journal fixtures, and transaction-bound PostgreSQL. It covers strict body and
idempotency handling, authentication/RBAC, disabled selector, concealed
cross-tenant access, invalid reason/state, durable replay/key conflict,
balanced reversal linkage, semantic audit, tenant isolation, and rollback.
The existing visibility preflight was verified before audit and request claim;
...
workflow action-reference, and actionlint gates PASS. No migration, hosted
Supabase, Storage, Railway, Vercel, credential, provider, or paid change
occurred. Keep journal-reverse flags/lists false/empty. Source evidence SHA:
...
two tenants, finance/viewer roles, draft invoices, and transaction-bound
PostgreSQL. It covers auth, strict empty-body and idempotency handling, viewer
denial, disabled selector, concealed cross-tenant access, draft-to-cancelled
state, durable replay/key conflict, semantic audit, tenant isolation, and
rollback. The first assertion was narrowed to the target invoice after the
fixture exposed unrelated tenant-B audit rows; no product defect was found.
...
## 2026-08-12 - M3.283 reconciliation statement reconcile Core write

- Changed the Web Core client, exact-tenant selector, reconcile Server Action,
  statement retry state, and local loopback harness/spec. The Nest workflow and
  database authority were reused; no new migration was required.
- Verified: focused Web Core/action tests 183/183; API build; Web typecheck and
  lint; disposable PostgreSQL/Nest HTTP canary 1/1; authenticated Playwright
  browser canary 1/1. The browser proved both line matches, confirmation-safe
  reconcile, strict body, idempotency key, rendered reconciled state,
  responsive overflow, zero console errors, and blocked external requests.
- Full gates passed: `pnpm test` (shared-types 332; database 241 passed/143
  skipped; API 764; Web 794), `pnpm typecheck`, `pnpm lint`, `pnpm build`
