...
- Source ledger: 119 migrations through `20260812100000`.
- Ledger shape: exact linear prefix; 64 missing, zero unexpected, zero applied
  after the first gap.
- Prior 2026-08-07 suffix scan reported 39 `drop-object`, 12 explicit
  ...
  Any release review must recompute them against all 64 pending files.
...
- M3.261 added a source-only tenant-scoped idempotency ledger for the
  fail-closed Core bank-statement auto-match command. The managed applied
  boundary remains 55 migrations through `20260729233017`; no hosted SQL or
  migration-history row changed.
...
The twelve manifest batches are review checkpoints only. They do not authorize
...
5. Apply all 64 migrations to the clone in source order, pausing only for
