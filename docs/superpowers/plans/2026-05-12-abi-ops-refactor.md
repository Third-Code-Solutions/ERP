# ABI Ops Refactor — Master Implementation Plan
...
**Goal:** Transform the existing "BuildOps" ERP foundation into "ABI Ops" — the unified fit-out operations platform for Actuate Builders Inc. as specified in `apps/web/REFACTOR.md`.
...
1. `pnpm --filter @buildops/database typecheck` passes
2. `pnpm --filter @buildops/shared-types typecheck && pnpm --filter @buildops/shared-types test` passes
3. `pnpm --filter @buildops/web typecheck` passes
4. `pnpm --filter @buildops/web build` produces no errors
...
7. Drizzle migration generated + applied (`pnpm --filter @buildops/database generate` then push to Supabase)
8. Phase commits: `git commit -m "feat(abi-ops): phase N — <module>"` + `git push origin main`

---

## Gap Map: Current → Target

| Domain | Current (BuildOps) | Target (ABI Ops) | Strategy |
|---|---|---|---|
...
The plan executes in 10 phases. Each is committed/pushed independently. Phases 1–7 align to ABI Ops modules M1–M7. Phases 0/8/9 are foundation + cross-cutting + polish.
...
- Modify: `package.json` (root name — keep "buildops" workspace name, but add display name)

**Rationale:** Don't rename the workspace packages (breaks imports across files). Update only user-visible "BuildOps" strings to "ABI Ops".

- [ ] Replace user-visible "BuildOps" in layout title, sidebar, login page header
...
### Task 0.3: Expand role enum to 9 ABI Ops roles

**Files:**
- Modify: `packages/database/src/schema/enums.ts` (role enum)
- Modify: `packages/auth/src/server.ts` (AppRole union + ROLE_RANK)
- New SQL migration: `supabase/migrations/20260512<HHMMSS>_abi_roles.sql`
...
- [ ] Generate migration: `pnpm --filter @buildops/database generate`
...
- [ ] Save this plan to `docs/superpowers/plans/2026-05-12-abi-ops-refactor.md` (done)
...
### Task 1.1: Update opportunity stage enum to 8-stage ABI Ops taxonomy
...
**Files:**
- New: `apps/web/src/lib/abi/won-conversion.ts`
...
- [ ] `git add . && git commit -m "feat(abi-ops): phase 1 — M1 CRM + Accounts + KYC + 8-stage pipeline"`
...
- [ ] Drizzle push
- [ ] Commit + push: `feat(abi-ops): phase 2 — M2 Proposal (PPRF + Inspection + Design)`
...
- [ ] Typecheck, build, E2E
- [ ] Drizzle push
- [ ] Commit + push: `feat(abi-ops): phase 3 — M3 BOM (Togal + Rate Cards + Client Portal + DocuSeal)`
...
3. **Database push cadence.** After each phase's schema changes: `pnpm --filter @buildops/database generate` → review SQL → apply to Supabase via `psql $DATABASE_URL -f <migration>`. Migration files committed alongside schema changes.
