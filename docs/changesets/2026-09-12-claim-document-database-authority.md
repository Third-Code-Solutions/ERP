...
## CI legacy grant fixture correction

PR 67's disposable Supabase CLI database exposed missing legacy SELECT/service grants: its post-migration `supabase-default-privileges.sql` cannot retroactively grant privileges to existing tables. The manual local replay had instead used `supabase-system-bootstrap.sql`, which establishes default grants before table creation. Reproduced the same nine failures in new synthetic loopback database `erp_claim_ci_grants_20260912` by replaying 171 migrations with the bootstrap's pre-creation default grants omitted, then applying the existing CI fixture. This is a controlled ACL reproduction, not a claim of identical Supabase CLI runtime.

With reviewed Agent 13 scope, extended only the CI fixture: authenticated SELECT on documents, progress_claims, tenants and progress_claim_documents; explicit service SELECT/INSERT/UPDATE/DELETE/TRUNCATE on those tables and projects. No client mutation, anonymous grant, production SQL, global permissive default, workflow ordering or audit grant was added. Audit service access already comes from the Cortex security migration and trigger authority is unchanged.

Added a rollback-only regression establishing representative legacy grants, executing both real candidate migrations without their outer transaction wrappers, then asserting preserved read/service access and client table/column denials. This independently catches an accidental migration revocation that a post-migration fixture might otherwise hide. All privilege changes from this test roll back.

Verification on the new loopback database: focused claim authority tests passed 16/16; combined claim and existing Cortex security tests passed 39/39, including server-only mutation and audit/client-denial gates. Database typecheck passed; `node --test scripts/verify-build-ops-invariants.test.mjs` passed 9/9 with schema-diff-before-fixture ordering intact. The authoritative Node 22/Supabase CLI CI rerun remains required. Existing retained databases were not reset or dropped; the new exact database name is retained for separately authorized cleanup. Hosted recovery/rehearsal holds remain unchanged.
