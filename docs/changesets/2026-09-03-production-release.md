...
| Database-backed Viewer policy alignment | `55cd12c87ca0be8dd65911b488d213d4e82fb133` | PARTIAL — run `33765209379` reduced 13 stale assertions to one: the Viewer revocation case passed, but its following cross-tenant assertion remained under the revoked role and was masked by the role guard. Same-tenant Viewer read parity, notification/Cortex mutation denial, and semantic opportunity audit assertions passed. |
| Cortex role/tenant boundary separation | candidate after `55cd12c87ca0be8dd65911b488d213d4e82fb133` | PENDING CI — restores the authorized role after proving revocation denial, then independently verifies foreign-tenant conversation concealment. |
| Client-safe top-bar authorization import | candidate after `166e400f37bbe78891137eeb39afde588ea29f39` | VERIFIED locally / PENDING CI — the top bar now consumes the shared pure capability policy directly instead of pulling the auth package's `next/headers` server module into a Client Component bundle; Node 22 production Web build compiled, type-checked, and generated all 89 static pages. |
| Production ledger reconciliation | seven versions through `20260901141949` | PENDING CI — six files recovered byte-for-byte from `agent-04/upload-reservations`; one CAD MIME allowlist statement recovered from the authenticated Supabase migration-history view. Production run `33768641038` executed no SQL and deployed no application. |
...
|---|---|---|---|
| Supabase identity | `aqqrtkmtcsfkbyyqxowv`; session pooler user `postgres.aqqrtkmtcsfkbyyqxowv`; port `5432` | Protected run `33768641038` passed exact-target verification for both redacted URLs | VERIFIED |
...
| Pending migration review | Additive only; tenant/RLS/audit/index/constraint review complete | Run `33768641038` found `0 missing` and seven already-applied live versions absent from `main`; source reconciliation is pending fresh replay and protected dry-run | BLOCKED until reconciliation PR passes |
...
| `2026-09-03T14:09:32Z` | Candidate CI | `55cd12c87ca0be8dd65911b488d213d4e82fb133` | FAILED before build/E2E | GitHub Actions run `33765209379`; `api-integration-vitest.json`, migration list, schema diff | Every non-database gate passed. PostgreSQL rebuild, ledger, RLS, schema diff, database tests, and 12 of 13 repaired Nest assertions passed; one cross-tenant assertion was masked by the preceding role-revocation setup and was separated before promotion. |
| `2026-09-03T14:17:52Z` | Candidate CI | `166e400f37bbe78891137eeb39afde588ea29f39` | FAILED at production Web build | GitHub Actions run `33766014215`; database reproducibility artifacts | All static, security, unit, PostgreSQL 17, RLS, schema, Nest integration, and API container gates passed. Webpack rejected the top bar's runtime import of the server-only auth barrel; trusted E2E was correctly skipped and the client-safe policy import was repaired before promotion. |
| `2026-09-03T14:32:34Z` | Candidate CI | `81b41f50c31e8d47da4312e7e8dd322354af627e` | PASSED | GitHub Actions run `33767459447` | Actionlint, invariants, security, lint, typecheck, full unit suite, PostgreSQL 17 reproducibility/integrations, production build, and trusted E2E all passed. |
| `2026-09-03T14:44:10Z` | PR merge | `main` | PASSED | PR `#29`; merge `4f2d1a247ac03e6f95f516815ba3156523e9a466` | Required PR path used; no direct push to `main`. |
| `2026-09-03T14:44:32Z` | Protected production promotion | Supabase / Railway / Vercel | FAILED CLOSED before provider deployment | GitHub Actions run `33768641038` | Release gates and exact Supabase target checks passed. Ledger reported 157 live / 150 source with seven unexpected versions and zero missing. No SQL, Railway deploy, Vercel deploy, or live credential mutation ran. |
