# M3.254 Supplier Bill Reverse HTTP Canary

## Scope

Added protected rollback-only HTTP evidence for the existing Core supplier-bill
reverse command. The canary exercises the real Nest controller/service,
...
- API integration: 49/49 files, 63 tests PASS; two Redis-restart cases are
...
Keep supplier-bill reversal writes disabled and the tenant allowlist empty
until hosted parity, release identity, readiness, protected browser evidence,
