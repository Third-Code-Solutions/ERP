...
- Pull request #13's CI security gates are blocked because GitHub Actions
  billing is unavailable. A skipped, disabled, or locally substituted security
  gate is not an acceptable release result.
...
repository/organization that owns PR #13. Once available, rerun the unchanged
PR #13 security gates from the clean port branch and retain the run URLs/IDs and
results. Do not disable, relax, reclassify, or replace gitleaks, SAST, dependency,
container, RLS, or other configured security gates to work around billing.
